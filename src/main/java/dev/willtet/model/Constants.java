package dev.willtet.model;

public class Constants {
    //Legends
    public static String APRENDIZ = "1231960763770605588";
    public static String ESCUDEIRO = "1231960822226354236";
    public static String CAVALEIRO = "1231960846058655855";
    public static String PALADINO = "1231960892711899156";
    public static String MESTRE = "1231960913922490390";


    //Sala importante
    public static String TESTE = "1272967732794494986";
    public static String CADASTRO_SQUAD = "1203053040802865225";
    public static String POST_PUBLISH = "1216898300297482320";

    public static String POST_LEGADO = "1272967732794494986";
    public static String QUEST_DIARIO = "1270072766619582635";

    public static String MODCOMANDO = "1285394394492698677";

    public static String CADASTRO_COD_INFLUENCER = "1353842581619871865";
}